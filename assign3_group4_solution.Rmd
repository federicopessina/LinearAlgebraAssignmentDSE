...
while (mcEpidemics != steadyState) {
  if (steps != 0) {
    mcEpidemics <-  initialDistribution * mcEpidemics ^ steps
  }
  steps <- steps + 1
}

print0("to reach the steady state the system needs ", steps, " steps")
```
...
ggplot(steps, aes(x = Iter, y = Value, col = Group))+
  ...
  xlab('Chain Step') +
  ylab('Probability') +
  ggtitle(paste0(manySteps,' Steps Chain Probability Prediction', collapse = NULL)) + theme(plot.title = element_text(hjust = 0.5))
